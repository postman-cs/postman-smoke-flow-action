...
### Mixed authentication by operation

Use `auth-plan-path` when one Smoke collection contains OAuth2, API key, or unauthenticated operations. The versioned YAML plan maps every active OpenAPI `operationId` to a request-level auth profile. OAuth tokens are cached only in runtime-local `pm.variables` and are discarded after the collection run. See [Mixed runtime authentication](docs/auth-plan.md) and the committed [example plan](examples/auth-plan.yaml).
...
| `auth-plan-path` | Optional repo-root-relative path to a version 1 YAML auth plan that maps operationId targets to request-level OAuth2, API key, or noauth profiles. Cannot be combined with auth-config-json. | no |  |
