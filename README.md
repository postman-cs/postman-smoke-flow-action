...
| `flow-path` | Optional repo-root-relative path to the curated flow.yaml manifest. When omitted under flow-mode auto, the action derives a smoke flow from spec-path; without spec-path (or under flow-mode off) the canonical Smoke collection is refreshed from the generated spec collection without flow curation. | no |  |
| `flow-mode` | Flow selection policy. auto (default) uses the curated flow.yaml when flow-path is set and otherwise derives a deterministic smoke flow from spec-path; curated requires flow-path; off disables curation entirely and refreshes the canonical Smoke collection from the generated spec collection. | no | `auto` |
